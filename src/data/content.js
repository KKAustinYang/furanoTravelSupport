...
const DEMOS=[
  ...
  {cat:'model', icon:'window', url:'/d/product-studio/index.html', ja:{t:'AI 商品撮影スタジオ（EC 商品ページ）',d:'商品写真1枚をAIが読み取り、商品名・価格・説明文と5枚ぶんの撮影プランを書き起こしたうえで、EC掲載用のプロ撮影5カット（白背景・俯瞰・シズルマクロ・モデル着用・季節ロケーション）を並列生成。季節は商談の場で選び直せ、従来の撮影費と納期を並べて示します。'},en:{t:'AI Product Photography Studio (E-commerce PDP)',d:'An LLM reads one product photo, writes the listing and art-directs five cuts, then white packshot, overhead flat lay, sizzle macro, in-use and seasonal location are generated in parallel — with the season re-pickable live and traditional shoot costs shown alongside.'},tags:['Modellix','EC','撮影']},
  ...
  {cat:'model', icon:'user', url:'/d/virtual-avatar/index.html', ja:{t:'ナレーション生成（バーチャルアバター）',d:'台本と参照音声から、指定した秒数ちょうどのナレーションを生成。約5秒ごとに分割して合成し、1本ずつ長さを検証してから無音を配って尺を合わせます。アバター動画化の前段となる音声工程のデモ。'},en:{t:'Narration Builder (Virtual Avatar)',d:'Turns a script plus a reference voice into narration that lands exactly on a target duration — split into ~5s chunks, each verified for length, then timed with weighted pauses. The voice stage that feeds avatar video.'},tags:['Modellix','音声','Live']},
  {cat:'model', icon:'layers', url:'/d/mangiro-staging/index.html', ja:{t:'バーチャルステージング（不動産・空室）',d:'空室の物件写真に、AIが家具を配置。部屋の広さに合わせて3パターンを生成し、Before / After のスライダーで見比べられます。間取りや壁・窓はそのまま、家具だけを足す不動産向けデモ。'},en:{t:'Virtual Staging for Vacant Listings',d:'AI furnishes a vacant room from its listing photo — three layouts per request, compared against the original with a before/after slider. The architecture stays untouched; only furniture is added.'},tags:['Modellix','不動産','ステージング']},
