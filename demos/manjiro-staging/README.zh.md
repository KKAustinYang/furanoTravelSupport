...
面向不动产平台 **MANJIRO**（运营方：KINAKO）的提案用 Demo。
把空房的房源照片交给 AI 摆上家具，客户在会议室里可以自己动手点、拖动 Before/After 对比。

- 页面：`demos/manjiro-staging/index.html`（单文件静态页，无需构建）
- 线上路径：`/d/manjiro-staging/index.html`
...
- 把样板房源换成实时生成
- 把 `MODELLIX_KEY` 交给浏览器（必须走同源 `/api` 代理，密钥只在服务端）
...
| `/d/staging/` | `/d/manjiro-staging/` | `demos/<slug>` 就是公开路径，slug 里保留客户名 |
| `public/staging/manifest.json` | `demos/manjiro-staging/staging/manifest.json` | 路径必须是**相对路径**，不能以 `/` 开头（页面挂在 `/d/` 下） |
...
```bash
node demos/manjiro-staging/tools/generate.mjs           # 只补缺的（幂等）
node demos/manjiro-staging/tools/generate.mjs --dry     # 只打印 prompt，不花钱
node demos/manjiro-staging/tools/generate.mjs --only prop-02
```
...
| ② ブランドロゴ | MANJIRO logo | 可（`STAGING_LOGO_DATA_URL` 覆盖，不用重新部署） |
...
```bash
node demos/manjiro-staging/tools/make-overlays.mjs   # 重写 api/_stamp-assets.js
```
...
- 新增 `demos/manjiro-staging/`（页面 + 47 张图 + 生成脚本 + 文档）
